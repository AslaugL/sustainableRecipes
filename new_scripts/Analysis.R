...
#Read data
data_recipes <- readRDS('C:/Users/aslau/Desktop/UiBV21/Master/Data/recipe_data_ready_for_analysis.Rds') #Nutrient content pr 100g recipe
data_ingredients <- readRDS('C:/Users/aslau/Desktop/UiBV21/Master/Data/ingredients_data_ready_for_analysis.Rds')
#Turn tidy
...
run_stats <- bind_rows(various$with_RDI, various$with_energy_pct_densityMJ) %>%
  ...
  getDescriptiveStats <- function(df, column){
    #Get descriptive stats of the third column in the df
...
formatKableStat <- function(df, features = NULL){
