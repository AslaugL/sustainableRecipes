...
To explore which nutrients that could be found together in a recipe, multiple graphs were built using tidygraph v. 1.2.0 [@Pedersen2020]. A graph is an object that contains nodes and edges, where nodes are variables and edges the connection between them. The edges may be directed or undirected, depending on if the relationship between the nodes are bidirectional or not [@Hevey2018]. For this analysis, the nodes in the graph were nutrients that a recipe was either a source or a good source of, with undirected edges connecting nutrients that were found in the same recipe. One graph was constructed for each of the recipe categories: Beef, lamb, game, pork, poultry, lean fish, oily fish, shellfish, vegetarian or vegan, and the number of recipes that were considered a source was normalized to the number of recipes in the recipe category. The graph objects were visualized using ggraph v. 2.0.5 [@Pedersen2021], using the default "stress" layout.
