...
formatKableStat <- function(df, features = NULL, caption = NULL){
  ...
  if(!is.null(caption)){
    kbl(dt, booktabs = TRUE, linesep = "", escape = FALSE) %>%
      kable_classic_2() %>%
      kable_styling(full_width = FALSE) %>%
      add_header_above(c(" " = 1, "Median (IQR)" = 3, "Kruskal-Wallis test" = 2, "Dunn test, Holm corrected" = 2))
  } else {
    kbl(dt, booktabs = TRUE, linesep = "", caption = caption, escape = FALSE) %>%
      kable_classic_2() %>%
      kable_styling(full_width = FALSE) %>%
      add_header_above(c(" " = 1, "Median (IQR)" = 3, "Kruskal-Wallis test" = 2, "Dunn test, Holm corrected" = 2))
  }
...
For all three countries, the median percentage of the weight of the recipe that was not mapped to a nutrient database or the SHARP Indicators database was 0, and the interquartile range were 0, 0. See **Figure** \@ref(fig:data-completeness)A and B.
...
nutrients_completeness <- plotViolinBox(data_completeness$no_nutrient_info) +
    ...
    labs(
      color = 'Country',
      x = 'Country',
      y = '% of recipe weight with no nutrient information'
    ) + ylim(0,100)
#Sustainability plot
sustainability_completeness <- plotViolinBox(data_completeness$no_sustainability_indicators) +
    ...
    labs(
      color = 'Country',
      x = 'Country',
      y = '% of recipe weight with no sustainability indicators'
    ) + ylim(0,100)
#Legend, right
legend <- get_legend(
...
stat_table %>%
  ...
  #Format table
  formatKableStat(caption = "Results from Kruskal-Wallis and post-hoc dunn test.") %>%
