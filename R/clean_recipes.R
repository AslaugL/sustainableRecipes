...
raw_list_NO <- list(
  'aperitif' = read_csv('./recipes/aperitif.csv'),
  'klikk' = read_csv('./recipes/klikk.csv'),
  'tine' = read_csv('./recipes/tine.csv'),
  'kolonial' = read_csv('./recipes/kolonialen.csv')
) %>%
  #Columns needed for my work
  bind_rows(.) %>% select(No, `Selected Meals`, Ingredients, Source, Country)

#Recipes needed
various$NO_recipes <- read_xlsx('./recipes/Data_NO_100.xlsx') %>%
...
#reread
raw$Norway <- read_xlsx('./recipes/all_norwegian_recipes.xlsx') %>%
  mutate(`Selected Meals` = `Selected Meals` %>%
           str_replace('PinnekjÃ¸tt', 'Pinnekjøtt') %>%
           str_replace('Biff with BearnÃ© saus', 'Biff with Bearné saus') %>%
...
#UK
raw$UK <- read_xlsx('./recipes/Data_UK_100.xlsx') %>%
  select(c(`Selected Meals`, Ingredients, `No. Of portion`, Source)) %>%
  mutate(Country = 'UK')  %>%
  drop_na(Ingredients)

raw$US <- readRDS('./recipes/US_clean.Rds') %>%
...
saveRDS(final_recipes, './output/recipes_data_ready_for_analysis.Rds')
...
saveRDS(final_ingredients, './output/ingredients_data_ready_for_analysis.Rds')  

#Save the ingredients with no amounts, without nutrient info and without sustainability indicators
missing_data <- list(
